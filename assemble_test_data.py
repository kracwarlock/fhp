...
example_dirname  = os.path.join('/ais/gobi3/u/shikhar/fhp')
training_dirname = os.path.join('/ais/gobi3/u/shikhar/fhp/data')
...
if __name__ == '__main__':
    ...
    csv_filename = os.path.join(example_dirname, 'test_dataset.csv')
    df = pd.read_csv(csv_filename)
    df = df.iloc[np.random.permutation(df.shape[0])]
    df['label'] = (df['label']*1000).astype(int)
    if args.labels > 0:
